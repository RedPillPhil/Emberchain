...
- [chain-node internal auth](chain-node-internal-auth.md) — CHAIN_NODE_INTERNAL_SECRET must be a Replit Secret; HMAC fallback from SESSION_SECRET; fail-closed if neither set.
...
- [Ember Delta API routing](ember-delta-api-routing.md) — use `${API}/api/*` (not `/api-server/api/*`); Replit proxy routes `/api/*` to api-server; `/api-server/` prefix hits Vite SPA fallback from browser.
- [Token Launch Architecture](token-launch-architecture.md) — 4 new contracts, backend DB+processor+routes, 4-step frontend wizard; needs UNIVERSAL_BRIDGE_ADDRESS env var to auto-deploy wrapped tokens.
...
- [Standalone ember-daemon](standalone-daemon.md) — lib/ember-daemon + Electron GUI; SQLite replaces PostgreSQL; better-sqlite3 needs onlyBuiltDependencies; Electron excluded from pnpm workspace.
