...
const pool = new pg.Pool({
  ...
  max: 10,                        // handle concurrent community polling
...
export async function ensureCommunityTables(): Promise<void> {
  try {
    await pool.query(`
      ...
      -- Index for the listPosts ORDER BY created_at DESC query.
...
export async function insertMessage(author: string, content: string): Promise<ChatMessage> {
  ...
  // Run INSERT and profile lookup concurrently — the profile read does not
  // depend on the insert result, only on the address which we already have.
  const [{ rows }, profile] = await Promise.all([
    pool.query<{ id: number; author: string; content: string; created_at: Date }>(
