...
async function setup(dir: string): Promise<TestCtx> {
  ...
  await bootstrap.flush(); // ensure async drainWriter finishes before readFileSync
...
test("block finder receives the full reward when no shares were submitted", async () => {
  ...
  try {
    ...
    const payoutDev = BigInt(block.payouts![DEV_ADDR] ?? "0");
    // The DEV BOOST always allocates a portion to the dev address, so the block
    // finder does not receive 100% of the reward even in the zero-share case.
    assert.ok(payoutC > 0n, "block finder must receive a positive payout");
    assert.equal(payoutC + payoutDev, BLOCK_REWARD, "block finder + DEV must equal full block reward");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("block finder C gets shareDifficultyDivisor (256) shares; A=3 and B=1 pre-submitted shares split the rest", async () => {
  ...
  try {
    const { chain, dataFile, walletA, walletB, walletC, template, blockTarget } = await setup(dir);
    await chain.flush(); // drain any pending createWallet() writes before reading file
    ...
    // DEV BOOST takes a cut, so miners + DEV = full block reward.
    assert.equal(payoutA + payoutB + payoutC + payoutDev, BLOCK_REWARD, "all payouts including DEV must sum to exactly 5 EMBR");
    // Relative ordering: A (3 shares) earns more than B (1 share); C (256 shares) earns most.
    assert.ok(payoutA > 0n, "miner A must receive a positive payout");
    assert.ok(payoutB > 0n, "miner B must receive a positive payout");
    assert.ok(payoutC > 0n, "block finder C must receive a positive payout");
    assert.ok(payoutA > payoutB, "miner A (3 shares) earns more than miner B (1 share)");
    assert.ok(payoutC > payoutA, "block finder C (256 shares) earns more than miner A (3 shares)");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("submitShare auto-promotes a block nonce and distributes payout proportionally", async () => {
  ...
  try {
    ...
    const payoutC   = BigInt(block!.payouts![walletC.address.toLowerCase()] ?? "0");
    const payoutDev = BigInt(block!.payouts![DEV_ADDR] ?? "0");
    const total     = payoutA + payoutB + payoutC + payoutDev;

    // DEV BOOST takes a cut — miners + DEV must sum to the full block reward.
    assert.equal(total, BLOCK_REWARD, "all payouts including DEV must sum to exactly 5 EMBR");
...
test("accumulated shares survive a server restart", async () => {
  ...
  try {
    ...
    await chain.flush(); // ensure async drainWriter finishes before readFileSync
