/**
 * Operator admin authentication for chain-node bridge admin routes.
 *
 * Accepts either:
 *  - x-admin-secret (CHAIN_NODE_INTERNAL_SECRET or SESSION_SECRET legacy)
...
export function requireOperator(req: Request, res: Response, next: NextFunction): void {
  if (!isOperator(req)) {
    res.status(401).json({ error: "Unauthorized — relayer operator signature or admin secret required" });
    return;
  }
  next();
}
