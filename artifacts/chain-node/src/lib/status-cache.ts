...
// 30-second TTL — the cache is already invalidated on every new block
// (see mining.ts), so a longer TTL just means we avoid re-computing
// during mining-burst windows where the event loop is busy.
