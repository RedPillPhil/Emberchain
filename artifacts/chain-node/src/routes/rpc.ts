/**
 * Ethereum JSON-RPC 2.0 — makes Emberchain visible to MetaMask and any
 * EVM-compatible wallet or tooling.
 */
...
function formatReceipt(tx: StoredTransaction) {
  ...
  return {
    ...
    status: tx.status === "success" ? "0x1" : "0x0", to: tx.to ?? null,
    transactionHash: tx.hash, transactionIndex: "0x0", type: "0x0",
  };
}
  ...
  switch (method) {
    ...
    case "eth_feeHistory": {
      ...
      return {
        ...
        baseFeePerGas: Array(6).fill(baseFee), gasUsedRatio: Array(5).fill(0.1),
        reward: Array(5).fill(["0x" + GAS_PRICE.toString(16)]),
      };
    }
    case "eth_blockNumber": { const s = await chain.getStatus(); return toHex(s.height); }
    case "eth_getBalance": { const w = await chain.getWallet(params[0] as PrefixedHexString); return toQuantity(w.balance); }
    case "eth_getTransactionCount": { const w = await chain.getWallet(params[0] as PrefixedHexString); return toHex(w.nonce); }
    ...
    case "eth_getBlockTransactionCountByNumber": { const block = await resolveBlock(params[0] as string); return toHex(block?.transactions.length ?? 0); }
    case "eth_getBlockTransactionCountByHash": { const block = await chain.getBlockByHash(params[0] as string); return toHex(block?.transactions.length ?? 0); }
    case "eth_getUncleCountByBlockHash": case "eth_getUncleCountByBlockNumber": return "0x0";
    case "eth_getTransactionByHash": { const tx = await chain.getTransaction(params[0] as string); return tx ? formatTx(tx) : null; }
    case "eth_getTransactionByBlockNumberAndIndex": {
      const block = await resolveBlock(params[0] as string);
      const tx = block?.transactions[parseInt(params[1] as string, 16)];
    ...
    case "eth_getTransactionReceipt": { const tx = await chain.getTransaction(params[0] as string); return tx ? formatReceipt(tx) : null; }
    case "eth_sendRawTransaction": {
      const raw = params[0] as string;
      let rawBytes: Uint8Array;
      try { rawBytes = hexToBytes(raw as PrefixedHexString); } catch { throw rpcError(-32602, "Invalid raw transaction hex"); }
      let parsed;
      try { parsed = createTxFromRLP(rawBytes, { common }); } catch (e) { throw rpcError(-32602, `Could not parse transaction: ${(e as Error).message}`); }
      if (!parsed.verifySignature()) throw rpcError(-32602, "Invalid transaction signature");
    ...
    case "eth_estimateGas": {
      const c = params[0] as { to?: string; from?: string; data?: string; value?: string } | undefined;
      const gas = await chain.estimateGas({ to: c?.to ?? null, data: c?.data, from: c?.from ?? null, value: c?.value ? BigInt(c.value) : 0n });
      return "0x" + gas.toString(16);
    }
    case "eth_getLogs": return [];
    case "eth_newFilter": case "eth_newBlockFilter": case "eth_newPendingTransactionFilter": return "0x1";
    case "eth_getFilterChanges": case "eth_getFilterLogs": return [];
    case "eth_uninstallFilter": return true;
    case "eth_subscribe": case "eth_unsubscribe": throw rpcError(-32601, "Subscriptions require a WebSocket connection");
    default: throw rpcError(-32601, `Method not supported: ${method}`);
  }
}
