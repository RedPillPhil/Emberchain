...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  // ── Auto-refresh: balance polling + AppState background pause ─────────
  useEffect(() => {
    if (!address) return;

    // Start data fetching immediately — don't wait for discovery
    ...
    const handleAppState = (state: AppStateStatus) => {
      if (state === 'active') {
        // Coming back to foreground — refresh immediately then restart interval
        if (!iv) {
          refreshBalance(address);
          iv = setInterval(() => refreshBalance(address), 15_000);
        }
      } else {
        // Background / inactive — stop polling to save battery and RPC slots
  ...
  const connectAndRefresh = useCallback(async (addr: string) => {
    ...
    // Fire data fetches and discovery in parallel — don't waterfall on discovery.
    // apiCall() internally calls discoverNode() which deduplicates concurrent
    // discovery requests via _discoverInFlight, so all three share one probe.
    await Promise.allSettled([
      refreshBalance(addr),
      refreshTransactions(addr),
      discoverNode().then((node) => {
        if (node) {
          setNodeUrl(node);
