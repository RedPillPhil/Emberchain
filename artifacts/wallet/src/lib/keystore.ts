 ...
 *
 * Also exports walletFromPrivateKey for client-side address derivation
 * (avoids a server round-trip to chain-node during wallet import).
 */
...
/**
 * Derive the Ethereum-style address from a 0x-prefixed private key, entirely
 * in the browser — no server call, no chain-node dependency.
 *
 * Algorithm (matches lib/chain-core/src/crypto.ts):
 *   1. secp256k1 uncompressed public key (65 bytes)
 *   2. drop the 0x04 prefix → 64 bytes
 *   3. keccak256 → take last 20 bytes → hex address
 */
export function walletFromPrivateKey(privateKeyHex: string): { address: string; privateKey: string } {
  const privBytes = hexToBytes(privateKeyHex);
  if (!secp256k1.utils.isValidPrivateKey(privBytes)) {
    throw new Error("Invalid private key");
  }
  const pubKey  = secp256k1.getPublicKey(privBytes, false); // uncompressed, 65 bytes
  const pubRaw  = pubKey.length === 65 ? pubKey.slice(1) : pubKey; // drop 0x04
  const hash    = keccak256(pubRaw);
  const addrBytes = hash.slice(-20);
  const address = "0x" + Array.from(addrBytes).map(b => b.toString(16).padStart(2, "0")).join("");
  // Normalise private key to 0x-prefixed lowercase hex
  const pk = privateKeyHex.startsWith("0x") ? privateKeyHex : `0x${privateKeyHex}`;
  return { address, privateKey: pk.toLowerCase() };
}
