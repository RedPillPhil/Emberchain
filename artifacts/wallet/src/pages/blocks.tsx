...
export default function BlockExplorer() {
  ...
  const { data: chain, refetch: refetchChain } = useGetChainStatus({ query: { refetchInterval: false } });
  const { data: blocks, isLoading: blocksLoading, refetch: refetchBlocks } = useListBlocks(
    { limit: 10 },
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    try { await Promise.all([refetchChain(), refetchBlocks(), refetchTxs()]); }
  ...
  return (
    <Shell requireWallet={false}>
      {/* ── Header ── */}
      <div className="border-b border-border pb-6 mb-6 flex items-start justify-between gap-4">
        <div>
        ...
        <button
          ...
          disabled={refreshing}
          className="mt-2 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest text-muted-foreground hover:text-foreground border border-border rounded-sm px-3 py-2 transition-colors hover:border-primary/40 disabled:opacity-50"
        >
