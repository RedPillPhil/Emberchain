...
export default function Dashboard() {
  ...
  // Auto-refresh every 30 s.  submit-block concurrency is now capped at 3 on
  // the server, so chain-node's event loop is free to handle these reads quickly.
  ...
  const { data: chainStatus, refetch: refetchChain } = useGetChainStatus({
    query: { refetchInterval: 30_000 }
  });

  const { data: miningStatus, refetch: refetchMining } = useGetMiningStatus({
    query: { refetchInterval: 30_000 }
  });
          ...
          <div className="p-6 relative z-10">
            <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2 flex items-center gap-2">
              ...
              <button
                ...
                className="ml-auto text-muted-foreground hover:text-primary transition-colors disabled:opacity-40"
              >
            ...
            >
              {wallet ? formatEmbr(wallet.balance) : "—"}
            </div>
