...
function BridgeTokenPicker({
  selected,
  listings,
  onSelect,
}: {
  selected: "EMBR" | BridgeListing;
  listings: BridgeListing[];
  onSelect: (t: "EMBR" | BridgeListing) => void;
}) {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");

  const label = selected === "EMBR" ? "EMBR" : (selected as BridgeListing).symbol;
  const sub   = selected === "EMBR" ? "EmberChain native" : (selected as BridgeListing).token_name;

  const filtered = [
    { id: "EMBR", symbol: "EMBR", token_name: "EmberChain native" } as BridgeListing,
    ...listings,
  ].filter((l) =>
    !query ||
    l.symbol.toLowerCase().includes(query.toLowerCase()) ||
    l.token_name.toLowerCase().includes(query.toLowerCase()),
  );
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full bg-card border border-border rounded-sm shadow-xl overflow-hidden">
          <div className="p-2 border-b border-border">
            <div className="flex items-center gap-2 bg-secondary/60 rounded-sm px-2 py-1">
              ...
              />
              {query && (
                <button onClick={() => setQuery("")}><X className="w-3 h-3 text-muted-foreground" /></button>
              )}
            </div>
          </div>
          <div className="max-h-52 overflow-y-auto divide-y divide-border/40">
            {filtered.map((l) => (
              ...
              >
                ...
                {(selected === "EMBR" ? l.id === "EMBR" : (selected as BridgeListing).id === l.id) && (
                  <CheckCircle2 className="w-3.5 h-3.5 text-primary ml-auto shrink-0" />
                )}
              </button>
            ))}
            {filtered.length === 0 && (
              <div className="px-4 py-3 text-xs text-muted-foreground text-center">No tokens found</div>
            )}
          </div>
        </div>
      )}
...
function BridgeTab() {
  ...
  useEffect(() => {
    apiFetch("/api/token-launch/listings")
      .then((data) => {
        const live = (data as BridgeListing[]).filter((l) => l.status === "live");
        setListings(live);
      })
      .catch(() => {});
  }, []);
    ...
    <div className="max-w-lg mx-auto w-full space-y-6">
      ...
      <div className="space-y-1">
        ...
        <BridgeTokenPicker
          ...
          listings={listings}
          onSelect={(t) => { setBridgeToken(t); setAmount(""); setBridgeStatus(null); }}
        />
      </div>

      {/* ── Launched-token bridge view ── */}
      {bridgeToken !== "EMBR" && (
        <Card className="border-border bg-card/80 rounded-sm">
          <CardContent className="p-6 space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-primary/20 border border-primary/40 flex items-center justify-center text-sm font-bold text-primary">
                {(bridgeToken as BridgeListing).symbol.slice(0, 2)}
              </div>
              <div>
                <div className="font-bold text-foreground">{(bridgeToken as BridgeListing).symbol} Bridge</div>
                <div className="text-xs text-muted-foreground">{(bridgeToken as BridgeListing).token_name}</div>
              </div>
            </div>

            {(bridgeToken as BridgeListing).universal_bridge_address || (bridgeToken as BridgeListing).native_bridge_address ? (
              <div className="space-y-3">
                {(bridgeToken as BridgeListing).native_bridge_address && (
                  <div className="bg-secondary/50 border border-border rounded-sm p-3 space-y-1">
                    <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                      Native Bridge Contract ({(bridgeToken as BridgeListing).chain_name ?? "Source chain"})
                    </div>
                ...
                {(bridgeToken as BridgeListing).wrapped_token_address && (
                  <div className="bg-secondary/50 border border-border rounded-sm p-3 space-y-1">
                    <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                      Wrapped Token on Base (w{(bridgeToken as BridgeListing).symbol})
                    </div>
                    <div className="font-mono text-xs text-foreground break-all">
                      {(bridgeToken as BridgeListing).wrapped_token_address}
                    </div>
                ...
                <div className="flex items-start gap-2 text-xs text-muted-foreground bg-secondary/40 border border-border rounded-sm px-3 py-2">
                  <Info className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                  <span>
                    Send {(bridgeToken as BridgeListing).symbol} to the native bridge contract above.
                    The relayer will mint w{(bridgeToken as BridgeListing).symbol} on Base within ~2 minutes.
                  </span>
                </div>
              </div>
            ) : (
              <div className="flex items-start gap-3 bg-amber-500/10 border border-amber-500/30 rounded-sm p-4">
                ...
                <div className="text-sm text-amber-300/90">
                  <span className="font-bold block">Bridge being deployed</span>
                  The cross-chain bridge for {(bridgeToken as BridgeListing).symbol} is being set up.
                  Check back soon — contract addresses will appear here once live.
                </div>
              </div>
            )}
          </CardContent>
