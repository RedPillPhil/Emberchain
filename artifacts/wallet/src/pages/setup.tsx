...
import { decryptKeystore, parseKeystoreFile, walletFromPrivateKey } from "@/lib/keystore";
...
export default function Setup() {
  ...
  // Address derivation is pure secp256k1 crypto — no server call needed.
  ...
  const handleImport = (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const wallet = walletFromPrivateKey(importKey);
      setActiveWallet({ address: wallet.address, privateKey: wallet.privateKey });
    } catch (err: any) {
      setError(err?.message ?? "Failed to import wallet — check your private key.");
    }
  };
  ...
  const handleKeystoreImport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Derive address client-side — no chain-node round-trip needed.
