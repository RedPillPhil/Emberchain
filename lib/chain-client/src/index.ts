...
/**
 * Internal calls (/api/internal/*) bypass the persistent pool and use plain
 * fetch with an AbortController timeout.  This prevents the pool from entering
 * a permanently-stuck state when chain-node is temporarily busy under mining
 * load — the pool's keep-alive connections can all be in-flight and block
 * subsequent requests indefinitely, producing a misleading "fetch failed" error
 * instead of a clean timeout.  Non-internal calls still use the pool for
 * connection reuse efficiency.
 */
const INTERNAL_TIMEOUT_MS = 20_000; // 20 s — generous for a note scan
...
async function post<T>(path: string, data?: unknown): Promise<T> {
  ...
  const res = isInternal
    ? await fetchWithTimeout(url, init, INTERNAL_TIMEOUT_MS)
    : await fetch(url, {
        ...init,
        // @ts-expect-error — undici dispatcher is not in standard RequestInit types
