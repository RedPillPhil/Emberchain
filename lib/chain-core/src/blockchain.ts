...
export const EMBERCHAIN_CONFIG: ChainConfig = {
  ...
  genesisDifficulty: MIN_DIFFICULTY.toString(), // 50 M — matches the hard floor
  difficultyAdjustmentWindow: 10,  // 10-block window: fast enough to react, smooth enough to not whipsaw
...
export class Blockchain {
  ...
  /** O(1) txHash → block lookup — populated at load and maintained on every block push. */
  private txHashToBlock: Map<string, StoredBlock> = new Map();
  /**
   * Short-lived per-block RPC cache for getWallet (serves eth_getBalance /
   * eth_getTransactionCount).  Keyed by lowercase address. Cleared on every
   * new block commit so callers always see current state within one block time.
   */
  private rpcCache: Map<string, { balance: bigint; nonce: number }> = new Map();
  ...
  /**
   * Throttle guard for share-state persistence.
   * persist(false) on every share runs JSON.stringify on the entire chain state
   * synchronously, freezing the event loop at high share rates (20+ shares/sec).
   * Instead we schedule a single deferred persist 15 s after the last share;
   * blocks always persist immediately through applyBlock regardless.
   */
  private sharePersistTimer: ReturnType<typeof setTimeout> | null = null;
  ...
  private async init(): Promise<void> {
    ...
    if (!persisted) {
      // Wait for any same-process pending write to complete before reading
      // from disk.  In tests this avoids a race where a previous Blockchain
      // instance has a drainWriter in flight when the next instance loads.
      // In a real multi-process restart, this is always a no-op.
      await flushChainFile(this.dataFile);
    ...
    if (persisted) {
      // Clamp to MIN_DIFFICULTY in case the persisted value is below the current
      // floor (e.g. chain was previously at the old 1 000 minimum).
      this.difficulty = BigInt(persisted.difficulty) < MIN_DIFFICULTY
        ? MIN_DIFFICULTY
        : BigInt(persisted.difficulty);
      ...
      for (const block of this.blocks) {
        ...
        for (const h of block.transactionHashes) this.txHashToBlock.set(h, block);
      }
  ...
  /**
   * Waits for any in-flight file write to complete.  Useful in tests that
   * simulate a server restart in the same process: call this before reading
   * chain.json synchronously to avoid racing the async drainWriter.
   * In real multi-process restarts the writers map is empty so this resolves
   * immediately.
   */
  ...
  async createWallet(importPrivateKey?: string | null) {
    ...
    this.rpcCache.delete(wallet.address.toLowerCase());
  ...
  async getWallet(address: PrefixedHexString) {
    ...
    if (cached) return { address, balance: cached.balance.toString(), nonce: cached.nonce };
    const balance = await getBalance(this.stateManager, address);
    const nonce = await getNonce(this.stateManager, address);
    this.rpcCache.set(key, { balance, nonce });
  ...
  /** Look up the block that contains a given transaction hash. O(1) via index. */
  getBlockForTx(txHash: string): StoredBlock | undefined {
    return this.txHashToBlock.get(txHash);
  }
  ...
  async listBlocks(limit = 20): Promise<StoredBlock[]> {
    await this.whenReady();
    // this.blocks is append-only in ascending order — slice the tail and reverse
    // to get the N most recent blocks in O(limit) instead of O(N log N).
    return this.blocks.slice(-limit).reverse() as StoredBlock[];
  }
  ...
  async submitShare(params: {
    ...
    // Throttled share-state persistence: schedule one persist 15 s after the
    // last accepted share rather than calling persist() on every share.
    // JSON.stringify on the full chain state is synchronous and blocks the
    // event loop — at 20+ shares/sec that was freezing every other request.
    // Blocks always persist immediately through applyBlock so financial state
    // is never at risk; worst case on crash is losing the last 15 s of share
    // counts (not credits — those are settled per block, not per share).
    if (this.sharePersistTimer) clearTimeout(this.sharePersistTimer);
    this.sharePersistTimer = setTimeout(() => {
      this.sharePersistTimer = null;
      this.persist(false);
    }, 15_000);
  ...
  private async applyBlock(
    ...
    for (const tx of included) {
      ...
      // Yield to the macrotask queue before each transaction so the HTTP server
      // can process pending requests (e.g. mining/template) that arrived while
      // this block is being committed.  Promise-based awaits only yield to the
      // microtask queue; setImmediate is the only way to let I/O through.
    ...
    // Cancel any pending deferred share-persist — the persist() below covers it.
    if (this.sharePersistTimer) { clearTimeout(this.sharePersistTimer); this.sharePersistTimer = null; }
  ...
  private async reorgTo(newTip: StoredBlock, newTipTxs: StoredTransaction[]): Promise<void> {
    return this.withEvmLock(async () => {
      ...
      this.txHashToBlock.clear();
      for (const b of newCanonical) {
        for (const h of b.transactionHashes) this.txHashToBlock.set(h, b);
      }
      this.rpcCache.clear();
  ...
  async privateSend(input: {
    ...
    return this.runExclusive(async () => {
      ...
      this.rpcCache.delete(PRIVACY_FEE_SINK_ADDRESS.toLowerCase());
  ...
  async unshield(input: { fromPrivateKey: string; toAddress: string; amount: string }): Promise<ShieldedTxRecord> {
    ...
    return this.runExclusive(async () => {
      ...
      this.rpcCache.delete((input.toAddress as string).toLowerCase());
  ...
  async createListing(input: {
    ...
    this.rpcCache.delete(sellerAddress.toLowerCase());
  ...
  async cancelListing(id: string, sellerPrivateKey: string): Promise<ExchangeListing> {
    ...
    this.rpcCache.delete(listing.sellerAddress.toLowerCase());
  ...
  async commitFulfillment(
    ...
    this.rpcCache.delete(buyerAddress.toLowerCase());
  ...
  async devFaucet(address: string, amountWei: bigint): Promise<void> {
    ...
    this.rpcCache.delete(address.toLowerCase());
  }
}
