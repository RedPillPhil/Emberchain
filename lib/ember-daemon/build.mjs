...
const EXTERNAL = [
  // Native addons (must be installed as real npm packages)
  "*.node", "better-sqlite3",
  // Rarely-needed optionals that bloat the bundle
  "sharp", "canvas", "bcrypt", "argon2", "fsevents", "re2", "farmhash",
  "xxhash-addon", "bufferutil", "utf-8-validate", "ssh2", "cpu-features",
...
const BANNER = `import { createRequire as __crReq } from 'node:module';
...
async function buildAll() {
  ...
  await Promise.all([
    // CLI entry point  (emberd binary)
    esbuild({ ...shared, entryPoints: [path.resolve(pkgDir, "src/cli.ts")] }),
    // Embeddable API  (used by Electron main process)
    esbuild({ ...shared, entryPoints: [path.resolve(pkgDir, "src/embedded.ts")] }),
  ]);
}
