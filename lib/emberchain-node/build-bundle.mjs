...
// ── Patch fileURLToPath — our SEA placeholder "file:///standalone" has no drive
// letter, which throws ERR_INVALID_FILE_URL_PATH on Windows. Intercept that URL
// and return an absolute path using the exe directory instead.
(function() {
  var _urlMod = require('url');
  var _orig   = _urlMod.fileURLToPath;
  _urlMod.fileURLToPath = function(u) {
    var s = (u && typeof u === 'object' && u.href) ? u.href : String(u);
    if (s === 'file:///standalone') {
      // Return exe-relative absolute path — consistent on Win + Unix
      return __path.join(__dirname, 'standalone');
    }
    try { return _orig.call(this, u); } catch(_e) { return __dirname; }
  };
})();
...
process.on('unhandledRejection', function(r){ __fatal(r instanceof Error ? r : new Error(String(r))); });`,
  ...
  alias: {
    ...
    "pino-http": path.resolve(__dirname, "src/stub-pino-http.ts"),
  },
  define: {
