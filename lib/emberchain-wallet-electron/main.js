...
const { tryUPnP, getLocalIp } = require('./upnp');
...
let upnpResult  = null;   // set after UPnP attempt when node starts
...
/** Persist whichever node won the election so the next launch skips the probe. */
...
/**
 * Select the best remote node.
 *
 * Fast path (normal startup / unlock):
 *   Try the previously-winning node (persisted as `cachedNode` in settings)
 *   with a single probe. If it responds, use it immediately — no parallel blast.
 *
 * Slow path (first launch or cached node is dead):
 *   Probe all candidates in parallel and elect the one with the highest block
 *   height (latency as tiebreaker). Persist the winner for next time.
 */
async function selectBestNode() {
  ...
  _selectInFlight = (async () => {
    try {
      ...
      // ── Fast path: try cached node first (single probe) ──────────────────
      if (cached) {
        ...
        // Cached node is dead — fall through to full election and clear it
        saveCachedNode(null);
      }

      // ── Slow path: full parallel election across all candidates ───────────
      ...
      saveCachedNode(_activeNode);   // persist winner for next launch
      return _activeNode;
    } finally {
...
async function smartRequest(reqPath, method = 'GET', body = null) {
  ...
  for (const base of candidates) {
    ...
    try {
      ...
      if (base !== _activeNode) {
        _activeNode = base;
        saveCachedNode(base);   // persist failover winner
  ...
  // All nodes failed — force re-probe next call, but don't wipe the cache:
  // the cache is only cleared when we know a node is definitively dead (above).
  _activeNode = null;
...
function startEmbeddedNode() {
  ...
  upnpResult = null;   // reset so UI shows "checking" while UPnP runs
  nodeEngine.startEmbeddedNode({ port: EMBEDDED_PORT, dataDir: DATA_DIR })
    .then(async () => {
      ...
      // Attempt UPnP port mapping in the background after the node port is open.
      // Asks the home router to open port 17545 and forward it to this machine so
      // other nodes can reach us without any manual router setup.
      // Works for ~70–80% of home routers. Fails gracefully to outbound-only mode.
      console.log('[main] Attempting UPnP port mapping…');
      try {
        upnpResult = await tryUPnP(EMBEDDED_PORT);
        if (upnpResult.mapped) {
          const publicUrl = `http://${upnpResult.externalIp}:${upnpResult.externalPort}`;
          console.log(`[main] UPnP success — publicly reachable at ${publicUrl}/api`);
          process.env.NODE_URL = publicUrl;
          // Persist so the URL is remembered across restarts
          const s = loadSettings();
          saveSettings({ ...s, publicUrl });
          // Announce our public URL to all known peers so other nodes can discover
          // us.  This is a one-shot parallel burst (not a repeating poll) — it just
          // tells the bootstrap nodes "I exist at this address" so they can return
          // us to other nodes during their regular PEX responses.
          if (nodeEngine?.announcePublicUrl) {
            nodeEngine.announcePublicUrl(publicUrl).catch(() => {});
          }
        } else {
          console.log(`[main] UPnP not available: ${upnpResult.reason}`);
        }
      } catch (err) {
        console.warn('[main] UPnP error:', err.message);
        upnpResult = { mapped: false, reason: err.message };
      }
      pushNodeStatus();
    })
    .catch(err => {
...
function pushNodeStatus() {
  ...
  try {
    ...
    // Pass UPnP result so renderer can show "Publicly Reachable" vs "Outbound Only"
    // and the appropriate help text.  null means UPnP check is still in progress.
    status.upnpResult = upnpResult;
...
ipcMain.handle('settings:set', (_e, patch) => {
  ...
  const s = { ...prev, ...patch };
  saveSettings(s);
  // Only invalidate the active node when the user explicitly changed the custom
  // node URL — other settings changes (theme, mining config, etc.) must not
  // trigger a fresh probe on the next API call.
...
ipcMain.handle('node:stop', async () => {
  ...
  if (nodeEngine && nodeRunning) {
    ...
    nodeRunning = false;
    _activeNode = null;
    upnpResult  = null;   // reset so UI shows idle state next time
  }
  pushNodeStatus();
  return { ok: true };
});
