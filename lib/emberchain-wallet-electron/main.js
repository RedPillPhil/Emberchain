...
ipcMain.handle('node:test-url', async (_e, url) => {
  ...
  try {
    ...
    // Got a response but it's not a chain node (e.g. a website).
    // Try appending /api and see if that works.
    try {
      const r2 = await nodeRequest(`${base}/api/sync/status`);
      if (r2.status === 200 && r2.body?.chainId) {
        return { ok: false, error: `That URL points to a website, not a node. Did you mean: ${base}/api ?` };
      }
    } catch { /* ignore */ }
    return { ok: false, error: `Reached a server but it doesn't look like an Emberchain node. Check the URL — it should end in /api (e.g. https://emberchain.org/api)` };
  } catch (err) {
    return { ok: false, error: err.message?.includes('timeout') ? 'Connection timed out — node may be busy or unreachable' : (err.message || 'Could not reach node.') };
  }
});
