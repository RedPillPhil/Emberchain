...
const FALLBACK_NODES = [
  ...
  'https://po-w-chain.replit.app/api',   // Replit mirror node
  // 'https://your-name.duckdns.org/api', // ← DuckDNS (add URL once known)
];
...
async function selectBestNode() {
  ...
  _selectInFlight = (async () => {
    try {
      ...
      // If the user saved a specific node, honour it without probing others
      if (settings.nodeUrl) {
        const probe = await probeNodeHeight(settings.nodeUrl.replace(/\/$/, ''));
        if (probe) { _activeNode = probe.base; return probe.base; }
      }
      const candidates = [...new Set(FALLBACK_NODES.map(u => u.replace(/\/$/, '')))];
...
app.whenReady().then(async () => {
  ...
  process.env.SEED_PEERS      = 'https://emberchain.org,https://po-w-chain.replit.app';
