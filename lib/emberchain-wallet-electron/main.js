...
async function selectBestNode() {
  ...
  _selectInFlight = (async () => {
    try {
      ...
      // User explicitly pinned a node — respect that choice
      if (settings.pinnedNode) {
        _activeNode = settings.pinnedNode.replace(/\/$/, '');
        return _activeNode;
      }
...
async function smartRequest(reqPath, method = 'GET', body = null) {
  ...
  // When the user has explicitly pinned a node, use only that node.
  if (settings.pinnedNode) {
    const base = settings.pinnedNode.replace(/\/$/, '');
    _activeNode = base;
...
function createWindow() {
  ...
  const menuTemplate = [
    { label: 'File', submenu: [
        { label: 'Node Settings…', accelerator: 'CmdOrCtrl+,', click: () => {
...
// Returns the known-nodes list with live probe results so the renderer can
// show a node picker with up-to-date block heights and latency.
ipcMain.handle('node:list', async () => {
  const settings = loadSettings();
  const pinned   = settings.pinnedNode?.replace(/\/$/, '') || null;
  const custom   = settings.nodeUrl?.replace(/\/$/, '')    || null;

  const allUrls = [...new Set([
    ...FALLBACK_NODES.map(u => u.replace(/\/$/, '')),
    ...(custom && !FALLBACK_NODES.map(u => u.replace(/\/$/, '')).includes(custom) ? [custom] : []),
  ])];

  const probes = await Promise.all(allUrls.map(async base => {
    const r = await probeNodeHeight(base);
    return {
      url:       base,
      ...
      online:    r != null,
      pinned:    base === pinned,
      active:    base === (_activeNode || '').replace(/\/$/, ''),
    };
  }));
...
// Pin the wallet to a specific node URL, or pass null/"auto" for auto-mode.
ipcMain.handle('node:set-pinned', async (_e, url) => {
  const s = loadSettings();
  if (!url || url === 'auto') {
    s.pinnedNode = null;
    _activeNode  = null;          // will re-probe on next request
    saveSettings(s);
    ...
    s.pinnedNode = base;
    _activeNode  = base;
    // Store as nodeUrl too so it shows up in the list next time
    if (!FALLBACK_NODES.map(u => u.replace(/\/$/, '')).includes(base)) {
      s.nodeUrl = base;
    }
    saveSettings(s);
  }
  mainWindow?.webContents?.send('node:active-changed', _activeNode);
  return { ok: true, active: _activeNode };
});
