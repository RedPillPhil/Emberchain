...
// ── Default node URL hint (shown in Settings, not used as an actual fallback) ─
const DEFAULT_NODE_HINT = 'http://localhost:8545/api';
...
ipcMain.handle('node:test-url', async (_e, url) => {
  ...
  try {
    ...
    return { ok: false, error: `Node responded with status ${r.status}` };
  } catch (err) {
    return { ok: false, error: err.message || 'Could not reach node.' };
  }
});
