...
contextBridge.exposeInMainWorld('ember', {
  ...
  getEmbeddedUrl:   ()       => invoke('node:embedded-url'),
  testNodeUrl:      (url)    => invoke('node:test-url', url),
  nodeStart:        ()       => invoke('node:start'),
  nodeStop:         ()       => invoke('node:stop'),
  nodeGetStatus:    ()       => invoke('node:status'),
  applyPublicUrl:   (url)    => invoke('node:apply-public-url', url),

  // Mining
  miningStart:      (opts)   => send('mining:start', opts),
  miningStop:       ()       => send('mining:stop'),
