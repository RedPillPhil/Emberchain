...
// ── Settings / Node picker ────────────────────────────────────────────────
let _pickerSelection = 'auto'; // 'auto' | node-url string | 'custom'
...
async function openSettings() {
  _pickerSelection = 'auto';
  $('node-custom-row').style.display = 'none';
  $('settings-node-url').value = '';
  $('settings-test-result').textContent = '';
  show($('overlay-settings'));

  // Show skeleton while probing
  $('node-picker-list').innerHTML = `
    <div style="color:#666;font-size:13px;padding:10px 0;">⏳ Probing nodes…</div>`;

  let data;
  try { data = await window.ember.listNodes(); }
  catch { data = { nodes: [], pinned: null, active: null, autoMode: true, customUrl: null }; }

  const { nodes, pinned, active, autoMode, customUrl } = data;

  // Pre-fill custom input if user had a non-known custom URL
  const knownUrls = nodes.map(n => n.url);
  if (customUrl && !knownUrls.includes(customUrl)) {
    $('settings-node-url').value = customUrl;
  }

  // Determine current selection for pre-checking radios
  if (autoMode) {
    _pickerSelection = 'auto';
  } else if (pinned) {
    _pickerSelection = knownUrls.includes(pinned) ? pinned : 'custom';
    if (_pickerSelection === 'custom') $('settings-node-url').value = pinned;
  }
  ...
  const rows = [
    ...
    buildNodeRow({
      value:   'auto',
      label:   'Auto — best block height',
      sublabel: active ? `Last connected: ${shortUrl(active)}` : 'Probes all nodes and picks the highest',
      dot:     'auto',
      checked: _pickerSelection === 'auto',
    }),
    // ── Known nodes ───────────────────────────────────────────────────
    ...nodes.map(n => buildNodeRow({
      value:   n.url,
      label:   shortUrl(n.url),
      sublabel: n.online
        ? `${(n.height ?? 0).toLocaleString()} blocks · ${n.latencyMs}ms`
        : 'Unreachable',
      dot:     n.online ? 'online' : 'offline',
      checked: _pickerSelection === n.url,
    })),
    // ── Custom row ────────────────────────────────────────────────────
    buildNodeRow({
      value:   'custom',
      label:   'Custom URL…',
      sublabel: 'Enter any EmberChain node address',
      dot:     'custom',
      checked: _pickerSelection === 'custom',
    }),
  ];

  $('node-picker-list').innerHTML = rows.join('');

  // Wire radio change
  $('node-picker-list').querySelectorAll('input[name="node-radio"]').forEach(radio => {
    radio.addEventListener('change', () => {
      _pickerSelection = radio.value;
      $('node-picker-list').querySelectorAll('.np-row').forEach(r =>
        r.classList.toggle('np-selected', r.dataset.value === _pickerSelection));
      const isCustom = _pickerSelection === 'custom';
      $('node-custom-row').style.display = isCustom ? 'block' : 'none';
      if (isCustom) $('settings-node-url').focus();
    });
  });

  // Show custom row if already on custom
  if (_pickerSelection === 'custom') $('node-custom-row').style.display = 'block';
}

function buildNodeRow({ value, label, sublabel, dot, checked }) {
  const dotStyle = {
    auto:    'background:linear-gradient(135deg,#f37021,#ffd580)',
    online:  'background:#4caf50',
    offline: 'background:#555',
    custom:  'background:#7c6af5',
  }[dot] || 'background:#555';

  return `<label class="np-row${checked ? ' np-selected' : ''}" data-value="${value}"
    style="display:flex;align-items:center;gap:12px;padding:10px 14px;
           border-radius:8px;cursor:pointer;border:1px solid ${checked ? '#f37021' : '#2e2e2e'};
           background:${checked ? 'rgba(243,112,33,.08)' : '#1e1e1e'};
           transition:border-color .15s,background .15s;user-select:none;">
    <input type="radio" name="node-radio" value="${value}" ${checked ? 'checked' : ''}
      style="display:none;">
    <span style="width:10px;height:10px;border-radius:50%;flex-shrink:0;${dotStyle}"></span>
    <span style="flex:1;min-width:0;">
      <span style="display:block;font-size:13px;font-weight:600;color:#e0e0e0;
                   white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${label}</span>
      <span style="display:block;font-size:11px;color:#666;margin-top:1px;">${sublabel}</span>
    </span>
    ${checked ? '<span style="color:#f37021;font-size:14px;">✓</span>' : ''}
  </label>`;
}
...
$('btn-settings-test').addEventListener('click', async () => {
  ...
  try {
    ...
    if (r.ok) {
      result.textContent = `✅ Block ${r.height?.toLocaleString() ?? '?'} · Chain ${r.chainId}`;
...
$('btn-settings-save').addEventListener('click', async () => {
  ...
  try {
    if (_pickerSelection === 'auto') {
      await window.ember.setPinnedNode('auto');
    } else if (_pickerSelection === 'custom') {
      const url = $('settings-node-url').value.trim().replace(/\/$/, '');
      if (!url) { alert('Enter a custom URL first.'); return; }
      await window.ember.setPinnedNode(url);
    } else {
      await window.ember.setPinnedNode(_pickerSelection);
    }
...
$('status-conn').addEventListener('click', openSettings);
window.ember.on('open:settings', openSettings);

// Update status bar when main process switches active node
window.ember.on('node:active-changed', url => {
  refreshStatus();
});
