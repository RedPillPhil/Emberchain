...
// ── Fallback-node toast ───────────────────────────────────────────────────
let toastTimer = null;
function showFallbackToast(nodeUrl) {
  $('toast-fallback-url').textContent = nodeUrl;
  show($('toast-fallback'));
  if (toastTimer) clearTimeout(toastTimer);
  toastTimer = setTimeout(() => hide($('toast-fallback')), 10000);
}

// Listen for main-process notification that we switched nodes
window.ember.on('node:switched', (newUrl) => {
  showFallbackToast(newUrl);
});
...
async function openSettings() {
  ...
  try {
    ...
    const el = $('settings-active-node');
    if (active && active !== s.nodeUrl) {
      el.textContent = `Currently connected via backup node: ${active}`;
    } else if (active) {
      el.textContent = `Currently connected to: ${active}`;
    } else {
      el.textContent = '';
    }
