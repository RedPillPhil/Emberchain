...
$('btn-refresh-shielded').addEventListener('click', async () => {
  ...
  try {
    ...
    const bal = res?.totalBalance ?? res?.balance ?? 0;
    $('balance-shielded').textContent = `${Number(bal).toLocaleString(undefined, {minimumFractionDigits:4,maximumFractionDigits:8})} EMBR`;
  } catch (err) {
    $('balance-shielded').textContent = 'Error';
  }
});
...
async function boot() {
  ...
  // Show unlock screen — try to read address from encrypted keystore for display
  showScreen('screen-unlock');
