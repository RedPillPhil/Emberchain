...
let myHashrate     = 0;
let netHashrate    = 0;
let lastNodeStatus = null;   // most recent node:status payload
...
async function refreshStatus() {
  try {
    ...
    if (height != null) {
      // Show whether we're connected to the local embedded node or a remote fallback
      const isLocal = lastNodeStatus?.usingLocalNode ?? lastNodeStatus?.running ?? false;
      conn.textContent = isLocal ? '⬤ Local Node' : '⬤ Remote Node';
      conn.className   = isLocal ? 'connected' : 'connected node-remote';
      $('status-block').textContent = `Block ${height.toLocaleString()}`;
    } else throw new Error('no height');
  } catch {
    const conn = $('status-conn');
    // If local node is starting up, say so instead of "Disconnected"
    if (lastNodeStatus?.starting || (lastNodeStatus?.running === false && !lastNodeStatus?.downloadError)) {
      conn.textContent = '⬤ Connecting…';
      conn.className   = 'node-syncing';
    } else {
...
function renderNodeStatus(s) {
  ...
  if (s.downloading) {
    ...
    ind.textContent = '⬤ Starting…'; ind.className = 'node-syncing';
  } else {
  ...
  if (s.downloading) {
    ...
    if (s.synced) {
      ...
      statusLabel.textContent  = s.peerCount === 0 ? '🟡 Running — No peers' : '🟢 Running — Synced';
    } else {
    ...
    $('node-sync-label').textContent = s.peerCount === 0 ? 'No peers — waiting for network…' : s.synced ? 'Fully synced' : 'Syncing blockchain…';
    ...
    hide($('node-downloading-msg'));
    statusDot.className    = 'node-dot dot-syncing';
    statusLabel.textContent  = '⏳ Starting local node…';
    $('node-sync-pct').textContent  = '0%';
    $('node-sync-bar').style.width  = '0%';
    $('node-sync-label').textContent = 'Waiting for node to start…';
    $('node-sync-detail').textContent = '';
  } else {
  ...
  // URLs — MetaMask RPC always points to the public endpoint (works from any
  // machine, works when the local node is off). Show the local node address as
  // a secondary row only while the embedded node is actually running.
  $('node-rpc-url').textContent = 'https://emberchain.org/api/rpc';

  const localRow = $('node-local-url-row');
  if (s.running) {
    $('node-local-url').textContent = s.localUrl || `http://127.0.0.1:${s.port}/api/rpc`;
    show(localRow);
  } else {
    hide(localRow);
  }

  // ── UPnP / reachability sections ──────────────────────────────────────
  // Four mutually-exclusive states drive four <div> sections in the HTML.
  const upnpIdle     = $('node-upnp-idle');
  const upnpChecking = $('node-upnp-checking');
  const upnpSuccess  = $('node-upnp-success');
  const upnpOutbound = $('node-upnp-outbound');

  if (!s.running && !s.starting) {
    // Node stopped — show idle explanation about UPnP
    show(upnpIdle); hide(upnpChecking); hide(upnpSuccess); hide(upnpOutbound);
  } else if (s.upnpResult === null || s.upnpResult === undefined) {
    // Node starting / UPnP check in progress
    hide(upnpIdle); show(upnpChecking); hide(upnpSuccess); hide(upnpOutbound);
  } else if (s.upnpResult?.mapped) {
    // UPnP success — show public URL
    hide(upnpIdle); hide(upnpChecking); show(upnpSuccess); hide(upnpOutbound);
    ...
    // UPnP failed — outbound-only mode with instructions
    hide(upnpIdle); hide(upnpChecking); hide(upnpSuccess); show(upnpOutbound);
  }
