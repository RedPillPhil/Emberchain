...
const DISCOVER_TTL = 6000; // ms — wait for all router replies
...
function fetchControlUrl(location) {
    ...
    const req = http.get(
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          for (const svcType of ['WANIPConnection', 'WANPPPConnection']) {
            const m = body.match(
              new RegExp(
...
function soapRequest(controlUrl, serviceType, action, body) {
  return new Promise((resolve, reject) => {
...
async function addPortMapping({ controlUrl, serviceType }, internalIp, internalPort, externalPort) {
  const ns = `urn:schemas-upnp-org:service:${serviceType}:1`;
  const body =
...
async function getExternalIp(result) {
 ...
 */
async function tryUPnP(internalPort) {
  try {
...
module.exports = { tryUPnP, getLocalIp };
